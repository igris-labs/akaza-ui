...
Unstyled does not mean empty: some parts include replaceable fallback content, such as a checkbox checkmark, disclosure chevron, or rating star. Replace these through their named slots. Sizes, borders, colors, typography, spacing, and disabled appearance belong in your application, not in a package theme.
...
These targets are not universally identical. For `Checkbox` and `Switch`, `class` and `ui.wrapper` target the outer wrapper; `ui.root` targets the interactive button. Use each component's Styling Hooks table to choose the exact element.
