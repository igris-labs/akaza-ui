    ...
    <div :class="exampleStack">
          ...
          <Tabs v-model="basic" :items="basicItems" aria-label="Main navigation" :ui="tabsUi">
            <template #panel-overview>
              <p class="text-sm text-muted-foreground">
                Overview — a summary of your project's key metrics and recent activity.
              </p>
            </template>
            <template #panel-analytics>
              <p class="text-sm text-muted-foreground">
                Analytics — detailed charts and breakdowns of usage data.
              </p>
            </template>
            <template #panel-settings>
              <p class="text-sm text-muted-foreground">
                Settings — configure preferences, integrations, and access control.
              </p>
            </template>
          </Tabs>
        </div>
          ...
          <Tabs v-model="pill" :items="pillItems" aria-label="Account settings" :ui="tabsPillUi">
            <template #tab="{ item, isActive }">
              <span
                ...
                :class="isActive ? 'bg-background text-foreground' : 'text-muted-foreground'"
              >
                {{ item.label }}
              </span>
            </template>
            ...
            <template #panel-security
              ><p class="text-sm text-muted-foreground">
                Update password and two-factor authentication settings.
              </p></template
            >
        ...
        <div :class="canvas">
          ...
          >
            ...
            <template #panel-appearance
              ><p class="text-sm text-muted-foreground">
                Pick a theme and adjust interface density.
              </p></template
            >
        ...
        <div :class="canvas">
          ...
          >
            <template #panel-active
              ><p class="text-sm text-muted-foreground">Showing active items.</p></template
            >
            <template #panel-paused
              ><p class="text-sm text-muted-foreground">Showing paused items.</p></template
            >
            <template #panel-archived
              ><p class="text-sm text-muted-foreground">Archived items.</p></template
            >
          </Tabs>
      ...
      <div>
        ...
        <div :class="canvas">
          ...
          >
            <template #panel-html
              ><p class="text-sm text-muted-foreground font-mono">
                Arrow keys move focus without switching tabs. Press Space or Enter to activate.
              </p></template
            >
            <template #panel-css
              ><p class="text-sm text-muted-foreground font-mono">CSS panel content.</p></template
            >
            <template #panel-js
              ><p class="text-sm text-muted-foreground font-mono">
          ...
          <p class="text-xs text-muted-foreground mt-3">
            Arrow keys navigate without activating. Space / Enter activates the focused tab.
          </p>
        </div>
      </div>
          ...
          >
            ...
            <template #panel-b="{ isActive }">
              <div v-if="isActive" @vue:mounted="onPanelBMount">
                <p class="text-sm text-muted-foreground">
                  Tab B mounted
                  <span class="font-semibold text-foreground">{{ mountCount }}</span> time(s).
                </p>
              </div>
            </template>
          </Tabs>
          <p class="text-xs text-muted-foreground mt-3">
            With <code :class="inlineCode">unmountOnHide</code>, Tab B is removed from the DOM when
            inactive. Switch away and back to see the mount counter increment.
          </p>
        </div>
      </div>
    </div>
